...
export class EkkoSwitch extends EkkoBase {
  ...
  get size(): SwitchSize {
    return (this.getAttribute('size') as SwitchSize) ?? 'md';
  }
  set size(val: SwitchSize) {
  ...
  #template(): string {
    return `
      <label class="label">
        <input part="base" class="input" type="checkbox" role="switch" />
...
if (!customElements.get('ekko-switch')) {
  customElements.define('ekko-switch', EkkoSwitch);
}
